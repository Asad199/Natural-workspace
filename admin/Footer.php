<?php 
include('connection.php');
extract($_REQUEST);
if(isset($send))
{
mysqli_query($con,"insert into feedback values('','$n','$e','$mob','$msg')"); 
$msg= "<h4 style='color:green;'>feedback sent successfully</h4>";
}
?>
<!-- Footer1 Start Here-->

<footer style="background-color: #a6ccff;">
    <div class="container-fluid">
  <div class="col-sm-4 hov">
    <img src="logo/logo2.png"width="400px"height="100px"/><br><br>
    <p class="text-justify">To meet today's critical business needs for more and more digital solutions, projects need to be smarter. Smart projects detect risks before they damage a project. They leverage repeatable best practices to enable teams to handle a higher volume of projects. Resource utilization is timely and accurate, so that it's easier to schedule and allocate the right team members to the right projects. Qualilty issues are flagged before errors are deployed. These are just a few of the many beneifts that stem from transforming the static information generated by today's mishmash of disconnected tools, into a modern, smart project platform: A Project Workspace.</p><br>
      <center><a href="about.php" class="btn btn-danger"><b>Read More..</b></a></center><br><br><br>
 <?php
  include('Social ican.php')
?>
  </div>&nbsp;&nbsp;
  <div class="col-sm-4 text-justify">
         <h3 style="color:#cdd51f;">Contact Us</h3>
      <p style="color:white;"><strong>Address:&nbsp;</strong>#6/2, 1st floor grape grand neelasandra banglore-47</p>
      <p style="color:white;"><strong>Email-Id:&nbsp;</strong>info@workspace.com</p>
      <p style="color:white;"><strong>Contact Us:&nbsp;</strong>(+91) 9008009801</p><br><br><br>
  </div>&nbsp;

  <!--Feedback Start Here-->
  <div class="col-sm-4 text-center">
      <div class="panel panel-primary">
        <div class="panel-heading">Feedback</div>
          <div class="panel-body">
            <?php echo @$msg; ?>
      <div class="feedback">
      <form method="post"><br>
        <div class="form-group">
          <input type="text" name="n" class="form-control" id="#"placeholder="Enter Your Name"required>
        </div>
        <div class="form-group">
          <input type="Email" name="e" class="form-control" id="#"placeholder="Email"required>
        </div>
        <div class="form-group">
          <input type="Number" name="mob" class="form-control" id="#"placeholder="Mobile Number"required>
        </div>
        <div class="form-group">
          <textarea type="Text" name="msg" class="form-control" id="#"placeholder="Type Your Massage"required></textarea>
        </div>
          <input type="submit" value="send" name="send" class="btn btn-primary btn-group-justified"required>
      </form>     
        </div>
       </div>
      </div>
    </div>

    <!--Feedback Panel Close here-->

  </div>
</footer>
<!--Footer1 Close Here-->

<!--Footer2 start Here-->

<footer class="container-fluid text-center"style="background-color:#000408;height:40px;padding-top:10px;color:#f0f0f0;">
  <p>Develope By Syed Asad Raza & Anushka | All Rights Reserved 2021</p>
</footer>

<!--Footer2 Close Here-->